    ...
    public void onReceiveMessageData(Context context, GTTransmitMessage msg) {
        ...
        try {
            ...
            if (jsonObject.optInt("notify", 0) == 1) {
                if (GeTuiManager.sTopPushMessageProvider != null) {
                    TopPushMessage message = new TopPushMessage();
                    message.setContent(data);
                    message.setPlatform(GeTuiManager.NAME);
                    GeTuiManager.sTopPushMessageProvider.onNotificationMessageClicked(context, message);
                }
            } else {
                if (GeTuiManager.sTopPushMessageProvider != null) {
                    TopPushMessage message = new TopPushMessage();
                    message.setContent(data);
                    message.setPlatform(GeTuiManager.NAME);
                    GeTuiManager.sTopPushMessageProvider.onReceivePassThroughMessage(context, message);
                }
            }
